    ...
    >
      <Link to={`/product/${product._id || product.id}`} className="block relative">
        <div className="relative pt-[100%] overflow-hidden bg-gray-50/50 dark:bg-gray-800/20 p-4">
          ...
          />
          {product.discount > 0 && (
            <div className="absolute top-3 left-3 bg-[#FF4747] text-white text-[10px] font-bold px-2 py-0.5 rounded-sm">
              -{product.discount}%
            </div>
          )}
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-x-4 group-hover:translate-x-0">
            <button className="bg-primary text-white p-1.5 rounded-full hover:bg-primary-hover shadow-sm">
               <ShoppingCart className="w-4 h-4" />
            </button>

             <button className="bg-white text-gray-500 p-1.5 rounded-full hover:text-primary shadow-sm border border-gray-100">
